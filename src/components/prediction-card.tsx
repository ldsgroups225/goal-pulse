...
export function PredictionCard({ 
  ...
  const bttsColor = bttsProb >= 0.65 
    ? 'bg-yellow-100 text-yellow-800 dark:bg-yellow-900/30 dark:text-yellow-300' 
    : 'bg-gray-100 text-gray-700 dark:bg-gray-800 dark:text-gray-300';
    
  const over25Color = over25Prob >= 0.65 
  ...
  return (
    ...
    >
      <Link href={`/${data.fixtureId}`} className="block flex-1 flex flex-col h-full">
        ...
        <div className="px-4 py-3 flex-1">
          ...
          {!isCompact && (
            <div className="flex items-center justify-between text-xs text-muted-foreground mt-3 bg-secondary/50 dark:bg-secondary/20 p-1.5 rounded-md">
              <div>
                Poss: {data.stats.possession.home}%
              </div>
              <div className="flex gap-1">
                <span>Shots: {data.stats.shots.home.total}-{data.stats.shots.away.total}</span>
                <span className="opacity-40">•</span>
                <span>Corners: {data.stats.corners.home}-{data.stats.corners.away}</span>
              </div>
              <div>
                Poss: {data.stats.possession.away}%
              </div>
            </div>
          )}
        </div>
