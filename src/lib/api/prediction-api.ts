import { LiveScoreResponse, Match, MatchPrediction } from "@/types";
...
function analyzeMatch(match: Match): MatchPrediction | null {
  try {
    ...
    return {
      ...
      prediction: {
        winProbability: {
          home: parseFloat(homeWinProb.toFixed(2)),
          draw: parseFloat(drawProb.toFixed(2)),
          away: parseFloat(awayWinProb.toFixed(2)),
        },
        recommendedBet,
        confidence: parseFloat(confidence.toFixed(2)),
        reasons,
        goals: {
          over15: parseFloat(over15Prob.toFixed(2)),
          over25: parseFloat(over25Prob.toFixed(2)),
          over35: parseFloat(over35Prob.toFixed(2)),
          btts: parseFloat(bttsProb.toFixed(2)),
        },
      },
      stats: {
        possession: {
        ...
        shots: {
          home: {
            ...
            offTarget: homeTeamStats?.shots?.offgoal || 0,
          },
          away: {
            total: awayShotsTotal,
            onTarget: awayShotsOnTarget,
            offTarget: awayTeamStats?.shots?.offgoal || 0,
          },
        },
        attacks: {
          home: {
